...
| Core workflow | Scan user-configured roots, inspect managed project/task state, create new managed projects, and work in project-rooted terminals |
| Value proposition | Reduce session switching by making project state, phase, URLs, and verification evidence visible in one place |
...
| Solo developer | Unsure what action should happen next | Reconstruct state from memory | Inspect task phase, queue, artifacts, and verification evidence from one place |
...
| P1 | Project files | Browse selected project files from the right detail column | Files tab shows a project-relative file tree, and editable text/code files can be opened in a new `vim` terminal tab with `nano` fallback |
| P1 | Git dirty files | Inspect changed files from the Git detail tab | Git tab shows a compact dirty file list and double-clicking a file opens a diff in a project-rooted terminal tab |
...
- Autonomous long-running background execution from the UI.
...
| Direct agent invocation is risky | Unintended file edits or long-running commands | Keep execution in user-controlled project terminals |
