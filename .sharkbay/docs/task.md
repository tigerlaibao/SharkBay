...
| `t-076-show-all-files-directories` | done | Show all files and directories | none | Files tab no longer hides entries by name; `.git`, `node_modules`, build output, hidden files, and `.env` files are visible |
| `t-075-collapsible-files-tree` | done | Make Files tree collapsible | none | Files tab directories use `+`/`-` controls and nested children are collapsed by default |
| `t-074-files-tab-nano-open` | done | Add Files tab with nano launch | none | Right detail tab lists selected project files and opens editable files in a new `nano` terminal tab |
| `t-073-uninstall-harness-action` | done | Add uninstall harness action | none | Right-click project action removes harness files and only matching `.gitignore` lines after confirmation |
| `t-072-wechat-cli-service-command` | done | Cover wechat-cli web service startup | none | Discovers and runs the `wechat-cli web --host 127.0.0.1 --port 8765 --no-token` virtualenv CLI service |
| `t-071-avatar-milk-background` | done | Use milk project background for avatar circles | none | Avatar circles now use `#fffdfa` in base themes and translucent milk-white in Night mode for transparent icon readability |
| `t-070-favicon-first-monorepo-icons` | done | Prefer favicons for monorepo web package icons | none | Monorepo `packages/web/public` and `apps/web/public` favicons now win over larger PWA/logo assets |
| `t-069-monorepo-project-icons` | done | Resolve monorepo web package project icons | none | Common `packages/web/public` and `apps/web/public` icon assets now resolve as local project avatar sources before runtime favicon URLs |
| `t-065-script-labeled-dev-services` | done | Use script-labeled dev service discovery | none | Root `dev:*` scripts and direct-child `scripts.dev` now produce script-labeled service pills with per-service cwd |
| `t-064-dev-service-pill-mvp` | done | Add dev service pill MVP | none | Root `package.json` `scripts.dev` projects now show a terminal-header `dev` service pill that starts a service log tab and stops/removes it |
| `t-063-sidebar-counts-terminal-add-button` | done | Remove sidebar counts and move terminal add button | none | Left sidebar section count badges were removed; the new-terminal plus button now sits at the far right of the terminal tabs row |
| `t-060-sharkbay-project-icon-source` | done | Use semantic project icon source for project avatars | none | Project avatar discovery now prefers `resources/project-icon.png` and no longer uses branded themed app-icon resources as generic project avatars |
| `t-059-project-avatar-scale-reset` | done | Reset project avatar image scale | none | Project avatar images are back to the base 100% size while preserving Night fallback icon color/filter fixes |
| `t-058-night-detail-chip-polish` | done | Soften Night detail chips and project counts | none | Night mode priority badges, Info chips, and Managed/Not setup count bubbles now use translucent styling; project avatar scaling is reduced to 108% |
| `t-057-night-pill-icon-polish` | done | Soften Night pills and default project icon | none | Night mode pill tags now use translucent semantic colors, fallback/bundled Shark project icons fill circular avatars better, and the Night terminal title is readable |
| `t-056-morning-theme-icons` | done | Rename Classic to Morning and regenerate app icons | none | Classic is now Morning in Settings/config, legacy `classic` configs migrate to Morning, and Morning/Day/Night icons were regenerated from the refreshed Downloads art with smaller macOS-style visual bounds |
| `t-055-classic-t050-palette` | done | Restore Classic to the T050 color baseline | none | Classic xterm now defines the full default ANSI palette so switching from Day/Night fully resets to the T050-compatible terminal colors |
...
| `t-023-terminal-title-strategy` | done | Improve terminal tab title strategy | `t-022-runner-task-registration` | Terminal tabs show project-relative cwd by default, long-running shell commands when useful, and stable titles for interactive foreground apps and monitors |
...
| --- | --- | --- |
| `t-087-exclude-service-tabs-from-activity-labels` | 2026-05-09 | Focused renderer workflow test, typecheck, build, full test suite, and `git diff --check` passed |
| `t-086-project-terminal-label-persistence` | 2026-05-09 | Focused renderer workflow test, typecheck, build, full test suite, and `git diff --check` passed |
| `t-079-workbench-file-git-polish` | 2026-05-09 | Focused Git/file tests, typecheck, build, full test suite, harness JSON parse, and `git diff --check` passed |
| `t-078-lazy-files-tree` | 2026-05-09 | Focused project file tests, typecheck, build, full test suite, and `git diff --check` passed |
| `t-077-files-tab-refresh-icons` | 2026-05-09 | `npm run typecheck`, `npm run build`, `git diff --check`, and full test suite passed |
| `t-076-show-all-files-directories` | 2026-05-09 | Focused project file tests, typecheck, build, full test suite, and `git diff --check` passed |
| `t-075-collapsible-files-tree` | 2026-05-09 | `npm run typecheck`, `npm run build`, `git diff --check`, and full test suite passed |
| `t-074-files-tab-nano-open` | 2026-05-09 | Focused project file tests, typecheck, full test suite, build, and `git diff --check` passed |
| `t-073-uninstall-harness-action` | 2026-05-08 | Focused uninstall tests, typecheck, full test suite, build, and `git diff --check` passed |
| `t-072-wechat-cli-service-command` | 2026-05-08 | Focused dev service tests, typecheck, build, full test suite, `git diff --check`, and real `../wechat-cli` scan probe passed |
| `t-071-avatar-milk-background` | 2026-05-08 | `npm run typecheck`, `npm run build`, scoped CSS source check, and `git diff --check` passed |
| `t-070-favicon-first-monorepo-icons` | 2026-05-08 | Focused scanner tests, `npm run typecheck`, `npm run build`, `git diff --check`, and real ItsMyLife scanner probe passed |
| `t-069-monorepo-project-icons` | 2026-05-08 | Focused scanner tests, `npm run typecheck`, `npm run build`, `git diff --check`, and real ItsMyLife scanner probe passed |
| `t-065-script-labeled-dev-services` | 2026-05-08 | Focused service/scanner tests, `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, and real AIBF/AIGF/ItsMyLife discovery probe passed |
| `t-064-dev-service-pill-mvp` | 2026-05-08 | Focused service/scanner/terminal tests, `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, and harness JSON parse checks passed |
| `t-063-sidebar-counts-terminal-add-button` | 2026-05-08 | `npm run typecheck`, `npm run build`, `git diff --check`, and harness JSON parse checks passed |
| `t-060-sharkbay-project-icon-source` | 2026-05-08 | `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, image checks, and scanner project-icon priority coverage passed |
| `t-059-project-avatar-scale-reset` | 2026-05-08 | `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, and source check for removed 108% scale passed |
| `t-058-night-detail-chip-polish` | 2026-05-08 | `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, and scoped CSS selector checks passed |
| `t-057-night-pill-icon-polish` | 2026-05-08 | `npm run typecheck`, `npm test`, `npm run build`, `git diff --check`, and scoped selector checks passed |
| `t-056-morning-theme-icons` | 2026-05-08 | `npm run typecheck`, `npm test`, `npm run build`, `npm run pack`, `git diff --check`, icon geometry checks, ICNS checks, and packaged resource checks passed |
